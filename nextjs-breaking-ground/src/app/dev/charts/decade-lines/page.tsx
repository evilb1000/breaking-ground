...
import DecadeLinesChartAnimated from "@/components/DecadeLinesChartAnimated"
...
export const metadata: Metadata = {
  title: "Decade lines preview",
  robots: {index: false, follow: false},
}

export default async function DecadeLinesPreviewPage({
  searchParams,
}: {
  ...
  const {rows, dataset} = loadActivePosterRows("decade-lines", root)
  ...
  return (
    <main className="min-h-screen max-w-[100vw] overflow-x-hidden bg-white">
      <article className="mx-auto w-full min-w-0 max-w-[686px] overflow-x-hidden px-5 py-12">
        <PosterThemePager typeId="decade-lines" themeSlug={theme.slug} href="/dev/charts/decade-lines" />
        <div className="bg-article-body break-words">
          ...
          <p className="mt-2 text-[13px]">
            <a className="underline" href="/dev/charts/decade-lines?root=washington_county_population_1950_2020">
              County population
            </a>
            {" · "}
            <a className="underline" href="/dev/charts/decade-lines?root=washington_county_township_growth_over_5pct">
              Township growth
            </a>
          </p>
        </div>
