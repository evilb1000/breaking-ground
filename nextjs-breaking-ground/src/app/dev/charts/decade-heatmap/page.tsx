...
import DecadeHeatmapChartAnimated from "@/components/DecadeHeatmapChartAnimated"
...
export const metadata: Metadata = {
  title: "Decade heatmap preview",
  robots: {index: false, follow: false},
}

export default async function DecadeHeatmapPreviewPage({
  searchParams,
}: {
  ...
  const {rows, dataset} = loadActivePosterRows("decade-heatmap")
  const copy = getAutumnPosterCopy(dataset?.rootName)
  ...
  return (
    <main className="min-h-screen max-w-[100vw] overflow-x-hidden bg-white">
      <article className="mx-auto w-full min-w-0 max-w-[686px] overflow-x-hidden px-5 py-12">
        <PosterThemePager typeId="decade-heatmap" themeSlug={theme.slug} href="/dev/charts/decade-heatmap" />
