...
3. **Configure Sanity:**
   ...
   **Verify connection:** With the Next.js app running, open `http://localhost:3000/api/sanity/connect` for a JSON health check. Or run:
   ```bash
   curl -s "https://y9xwdi89.api.sanity.io/v2024-01-01/data/query/production?query=count(*)"
   ```
   You should see `"result":<number>` (document count).
